...
namespace TrackingInfo.Controllers
{
    public class AutoFenceTimerController : Controller
    {
        ...
        [HttpGet]
        public JsonResult GetAutoFenceTimerGetByAccNoDeviceIDHour( string AccNo,string TrackID)
        {
            bool success = true;
            string _Message = string.Empty;
            List<AutoFenceTimerModel> data = new List<AutoFenceTimerModel>();
            try
            {
                data = AutoFenceTimerModel.Get_AutoFenceTimerGetByAccNoDeviceIDHour(AccNo, TrackID);
            }
            catch (Exception ex)
            {
                _Message = ex.Message;
